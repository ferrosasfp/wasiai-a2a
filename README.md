...
One asymmetry is better read early than discovered against a `400`: Solana is today the **outbound** rail (the gateway pays the agent) and the chain where it can debit a prepaid key. **Inbound** x402 on Solana exists in the code since WKH-314 and is **off in the deployment that is up right now**: with that rail's envs unset, `x-payment-chain: solana-devnet` answers `400 CHAIN_INBOUND_PAYMENT_UNSUPPORTED`, and with them set that same request answers `402` with a Solana challenge. Which of the two you are talking to is a field and not a promise: `chains[].acceptsInboundPayment` in `GET /capabilities`. Details in [Solana rail](#solana-rail).
...
What it does **not** do **in the deployment that is up right now**, stated with the real error: `x-payment-chain: solana-devnet` stops with `400 CHAIN_INBOUND_PAYMENT_UNSUPPORTED`, and the response spells out the asymmetry and the three ways out (another chain for the x402, a prepaid key, which does debit budget on `solana-devnet`, or asking the operator of that deployment to turn the rail on). That is a **configuration** statement, not a limit of the code: since WKH-314 the gateway can also charge inbound on Solana, and it does as soon as the four envs of that rail are set (`SOLANA_ADAPTER_ENABLED`, `SOLANA_X402_INBOUND_ENABLED`, `SOLANA_X402_INBOUND_PAY_TO`, `SOLANA_X402_INBOUND_CHALLENGE_SECRET`) — with them set the same request answers `402` with a Solana challenge instead of `400`. `acceptsInboundPayment()` in `src/adapters/registry.ts` is the single expression behind both the guard and what `GET /capabilities` publishes, so the honest way to know which of the two you are talking to is to read that field. See [doc/INTEGRATION.md](doc/INTEGRATION.md) for the full inbound-Solana flow.
...
|---|---|---|
| **Solana devnet** | sentinel 900001 | outbound always; inbound implemented and env-gated, USDC-SPL (`acceptsInboundPayment: false` **while the rail envs are unset**, which is today's deployment) |
...
The three EVM rows are checked by sending `x-payment-chain` to `POST /compose` with no payment: the `402` comes back with `eip155:2368`, `eip155:43113` or `eip155:84532` and the amount in that network token's decimals. The Solana row is checked the other way around **on today's deployment**: the same request with `x-payment-chain: solana-devnet` returns `400 CHAIN_INBOUND_PAYMENT_UNSUPPORTED` because that rail's envs are unset. Set them and the same request returns a `402` with a `solana:` network and a Solana challenge — the `400` is the state of the configuration, not of the code.
...
- **Inbound x402 on Solana: implemented (WKH-314), off in the deployment that is up.** The two claims again, separately. *In code*: the gateway issues a MAC'd Solana challenge, the payer executes the USDC-SPL transfer themselves and presents the signature, and the gateway verifies presence, finality, terms and binding against two RPC providers before granting once — there is no EIP-3009 on Solana, so nothing is signed on our side. *In the deployment that is up* (measured 2026-08-19: `curl -s "$GW/capabilities"` returns `{"key":"solana-devnet",…,"acceptsInboundPayment":false}`, next to `true` for the three EVM chains — no credentials needed, check it yourself before believing this line): the rail's four envs are unset, so `x-payment-chain: solana-devnet` stops with `400 CHAIN_INBOUND_PAYMENT_UNSUPPORTED` (`src/middleware/x402.ts`), a typed error that also names the two ways out: another chain for the x402, or a prepaid key to keep operating on `solana-devnet`. With the envs set, that same request answers `402`. The full flow, the envelope and every rejection code are in [doc/INTEGRATION.md](doc/INTEGRATION.md).
...
| Metric | Value |
|---|---|
| Test files | **316 test files** in the root suite. Derived from the `include` of `vitest.config.ts` over the git index, and checked in both READMEs, by `test/readme-numbers.test.ts` |
...
| Lint | `npm run lint` (Biome) over **516 files**, which is the `src/**/*.ts` of `biome.json` |

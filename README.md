...
- **Inbound x402 on Solana: implemented (WKH-314), off in the deployment that is up.** The two claims again, separately. *In code*: the gateway issues a MAC'd Solana challenge, the payer executes the USDC-SPL transfer themselves and presents the signature, and the gateway verifies presence, finality, terms and binding against two RPC providers before granting once — there is no EIP-3009 on Solana, so nothing is signed on our side. *In the deployment that is up*: the rail's four envs are unset, so `acceptsInboundPayment` is `false` and `x-payment-chain: solana-devnet` stops with `400 CHAIN_INBOUND_PAYMENT_UNSUPPORTED` (`src/middleware/x402.ts`), a typed error that also names the two ways out: another chain for the x402, or a prepaid key to keep operating on `solana-devnet`. With the envs set, that same request answers `402`. The full flow, the envelope and every rejection code are in [doc/INTEGRATION.md](doc/INTEGRATION.md).

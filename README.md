...
| Metric | Value |
|---|---|
| Test files | **292 test files** in the root suite. Derived from the `include` of `vitest.config.ts` over the git index, and checked in both READMEs, by `test/readme-numbers.test.ts` |
...
| Lint | `npm run lint` (Biome) over **485 files**, which is the `src/**/*.ts` of `biome.json` |

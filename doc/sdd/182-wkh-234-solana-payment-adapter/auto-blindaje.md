...
- **NOTA para AR/CR**: el threading LIVE de `settleCaip2` desde compose → `budget.debit` para el leg Solana NO se cableó (compose es W4, fuera del scope W5; el debit per-step usa el chainId inbound del caller). El mecanismo (budget.debit + columna + emit) está implementado y testeado a nivel unit (AC-8/AC-9); el cableado end-to-end es un follow-up fino.

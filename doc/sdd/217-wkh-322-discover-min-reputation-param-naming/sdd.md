...
| **TD-322-4** | `GET /discover?<nombre de 5000 chars>=1` escribe el `req.url` **entero** en la línea `incoming request` del logger de Fastify. La cota de `MAX_ECHOED_PARAM_NAME_LENGTH` acota la RESPUESTA, no el request, así que no toca esa línea | Medido en el AR-2 (MNR-1): `src/index.ts:95-97` usa `logger: { redact: REDACT_PATHS }` sin serializers custom ⇒ `req.url` completo. En POST **no hay** línea equivalente (el 400 sale por `reply.send()` sin log, y `event-tracking.ts:117` persiste la url sin query string) | Deuda declarada con nombre. Cerrarla es un serializer de request (`req.url.split('?')[0]` o un truncado), que es scope de una HU de observabilidad, no de esta. **Lo que esta HU sí hizo**: sacar del JSDoc la frase que la daba por resuelta |

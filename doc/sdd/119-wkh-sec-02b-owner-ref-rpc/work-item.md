...
- **SDD_MODE**: full
- **Estimación**: M
- **Branch sugerido**: `feat/119-wkh-sec-02b-owner-ref-rpc`
...
| 4 | `debit_with_dest_policy` (PERFORM interno) | SQL PERFORM | `supabase/migrations/20260606000000_a2a_key_spend_policies.sql:123` |

**Total: 4 callers** (1 TS directo + 3 SQL PERFORM).
...
- `src/services/delegation.ts` — NO modificar el cuerpo TS; `debit_delegation_and_parent`
  ya hace su propio Ownership Guard antes del PERFORM; solo se actualiza el PERFORM
  dentro de la migración SQL.
- `src/services/key-session.ts` — NO modificar el cuerpo TS (mismo caso que delegation).
...
**Opción A** (DROP + recrear — recomendada preliminarmente):
...
- Actualizar los 3 RPCs SQL dependientes (DROP + CREATE OR REPLACE según necesiten)
  en la MISMA migración (atómico).
...
- Crear `increment_a2a_key_spend_owned(p_key_id, p_chain_id, p_amount_usd, p_owner_ref)`
  que valida ownership y delega a la original.
...
**Recomendación preliminar del Analyst**: Opción A, dado que el patrón DROP ya
existe en el codebase (BLQ-MED-1) y la Opción B deja un vector de confusión.

[NEEDS CLARIFICATION — Architect F2]: ¿Opción A o B? Evaluar el riesgo de
downtime en Railway rolling deploy: el DROP es DDL transaccional en Postgres
(instantáneo), pero si hay llamadas en vuelo al momento del DROP pueden fallar.
El Architect debe decidir si aplica en ventana de mantenimiento o si el tráfico
en ese path es lo suficientemente bajo para permitir hot-apply.
...
de `20260606000000_a2a_key_spend_policies.sql`). El `IF NOT FOUND` existente ya
cubre `KEY_NOT_FOUND`; el guard de ownership es un check adicional sobre la fila
lockeada.

### DT-3 — `p_owner_ref` en los PERFORM SQL internos

Los RPCs SQL que llaman `PERFORM increment_a2a_key_spend(...)` ya reciben
`p_owner_ref` como parámetro propio (lo usan para su propio Ownership Guard).
Por lo tanto el único cambio SQL en esos RPCs es agregar `p_owner_ref` al PERFORM:
`PERFORM increment_a2a_key_spend(p_key_id, p_chain_id, p_amount_usd, p_owner_ref)`.
La lógica de negocio de esos RPCs no cambia.

### DT-4 — Cómo obtiene `p_owner_ref` el caller #1 (budget.ts ruta master) [NEEDS CLARIFICATION]
...
ruta master, el Architect DEBE decidir entre:

(a) Mismo SELECT cold-path que ya existe en la ruta con destino (mínima
    superficie de cambio, sin impacto en call-sites).
(b) Ampliar la firma de `debit()` para incluir `ownerId: string` como parámetro
    obligatorio (más correcto arquitectónicamente, pero requiere actualizar todos
    los call-sites de `budgetService.debit`).
(c) Refactorizar `debit()` para que `ownerId` siempre sea obligatorio (alinea
    con la Security Convention de CLAUDE.md §"Cómo obtener el ownerId").
...
- **CD-3**: PROHIBIDO propagar el mensaje crudo de Postgres `OWNERSHIP_MISMATCH`
  ...
  en `budget.ts` (mismo patrón que `debit_with_dest_policy`, líneas 275-278 de
...
- **[NEEDS CLARIFICATION — Architect F2]** DT-1: ¿Opción A (DROP+recrear,
  recomendada) o Opción B (nueva función wrapper)? Evaluar riesgo de downtime en
  Railway rolling deploy.
- **[NEEDS CLARIFICATION — Architect F2]** DT-4: ¿Cómo obtiene `ownerId` el
  caller #1? ¿SELECT cold-path, ampliar firma de `debit()`, o refactor completo?
  El impacto en call-sites de `budgetService.debit` (compose.ts, orchestrate.ts,
  etc.) debe ser evaluado en F2 con grounding de todos los callers.
...
- Esta HU NO bloquea otras HUs actualmente en pipeline.
...
- Puede ir en paralelo con WKH-SEC-02c (NNN 118 — RLS en
  `registries`/`kite_schema_transforms`) ya que no tocan los mismos archivos.
- Branch: `feat/119-wkh-sec-02b-owner-ref-rpc` (aislado, sin conflicto con main
  ni con la rama de WKH-SEC-02c).

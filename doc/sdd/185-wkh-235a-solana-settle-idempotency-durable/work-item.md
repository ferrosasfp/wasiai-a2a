...
- **Commitment de `verify()` desalineado con la config (MNR-2 del AR, fix-pack 2026-07-25).** `verify()` (`src/adapters/solana/payment.ts`, `getParsedTransaction`) hardcodea `commitment: 'confirmed'` e **ignora** `getSolanaCommitment()` (`src/adapters/solana/chain.ts:45-48`), que sí se respeta en el `sendAndConfirmTransaction` del settle. Es **pre-existente** (WKH-234), pero la recuperación de firma de esta HU lo vuelve *load-bearing*: con `SOLANA_COMMITMENT=finalized`, un timeout a nivel **finalized** se recuperaría contra una lectura a nivel **confirmed** → el settle se declararía exitoso con una garantía **más débil que la configurada**. Impacto **bajo hoy** (devnet + `SOLANA_COMMITMENT` sin configurar ⇒ default `confirmed` ⇒ settle y verify coinciden). Fix cuando se reactive: pasar `getSolanaCommitment()` al `getParsedTransaction` de `verify()` (y decidir si el read debe ser >= al commitment del settle). Marcado con un comentario `REVISAR antes de mainnet` en el código.
  - **Condición de reactivación:** **antes de mainnet / dinero real**, o en cuanto se configure `SOLANA_COMMITMENT=finalized` en cualquier entorno.

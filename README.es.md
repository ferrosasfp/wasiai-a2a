...
Una asimetría que conviene leer temprano en vez de descubrirla contra un `400`: Solana es hoy el rail de **salida** (el gateway le paga al agente) y la cadena en la que puede debitar una clave prepaga. El x402 de **entrada** en Solana existe en el código desde WKH-314 y está **apagado en el deployment que está arriba ahora mismo**: con las envs de ese rail sin poner, `x-payment-chain: solana-devnet` contesta `400 CHAIN_INBOUND_PAYMENT_UNSUPPORTED`, y con ellas puestas ese mismo request contesta `402` con un challenge de Solana. Cuál de los dos tenés enfrente es un campo y no una promesa: `chains[].acceptsInboundPayment` en `GET /capabilities`. El detalle está en [Rail Solana](#rail-solana).
...
Lo que **no** hace **en el deployment que está arriba ahora mismo**, dicho con el error real: `x-payment-chain: solana-devnet` corta con `400 CHAIN_INBOUND_PAYMENT_UNSUPPORTED` y la respuesta explica la asimetría y las tres salidas (otra cadena para el x402, una clave prepaga, que sí debita presupuesto en `solana-devnet`, o pedirle al operador de ese deployment que encienda el rail). Eso es una afirmación sobre la **configuración**, no un límite del código: desde WKH-314 el gateway también sabe cobrar hacia adentro en Solana, y lo hace apenas están las cuatro envs de ese rail (`SOLANA_ADAPTER_ENABLED`, `SOLANA_X402_INBOUND_ENABLED`, `SOLANA_X402_INBOUND_PAY_TO`, `SOLANA_X402_INBOUND_CHALLENGE_SECRET`) — con ésas puestas, el mismo request contesta `402` con un challenge de Solana en vez de `400`. `acceptsInboundPayment()` (`src/adapters/registry.ts`) es la única expresión detrás del guard y de lo que publica `GET /capabilities`, así que la forma honesta de saber con cuál de los dos estados estás hablando es leer ese campo. El flujo completo está en [doc/INTEGRATION.md](doc/INTEGRATION.md).
...
|---|---|---|
| **Solana devnet** | sentinela 900001 | saliente siempre; entrante implementado y gateado por env, USDC-SPL (`acceptsInboundPayment: false` **mientras las envs del rail estén sin setear**, que es el deployment de hoy) |
...
Las tres filas EVM se comprueban mandando `x-payment-chain` a `POST /compose` sin pago: el `402` vuelve con `eip155:2368`, `eip155:43113` o `eip155:84532` y el monto en los decimales del token de esa red. La fila de Solana se comprueba al revés **en el deployment de hoy**: el mismo request con `x-payment-chain: solana-devnet` devuelve `400 CHAIN_INBOUND_PAYMENT_UNSUPPORTED` porque las envs de ese rail están sin setear. Con ésas puestas, el mismo request devuelve un `402` con una red `solana:` y un challenge de Solana — el `400` es el estado de la configuración, no el del código.
...
- **x402 entrante en Solana: implementado (WKH-314), apagado en el deployment que está arriba.** Otra vez las dos afirmaciones por separado. *En el código*: el gateway emite un challenge de Solana firmado con MAC, el pagador ejecuta él mismo la transferencia de USDC-SPL y presenta la firma, y el gateway verifica presencia, finalidad, términos y binding contra dos proveedores de RPC antes de conceder una sola vez — en Solana no hay EIP-3009, así que de nuestro lado no se firma nada. *En el deployment que está arriba* (medido el 2026-08-19: `curl -s "$GW/capabilities"` devuelve `{"key":"solana-devnet",…,"acceptsInboundPayment":false}`, al lado de `true` para las tres cadenas EVM — sin credenciales, comprobalo antes de creerle a esta línea): las cuatro envs del rail están sin setear, así que `x-payment-chain: solana-devnet` corta con `400 CHAIN_INBOUND_PAYMENT_UNSUPPORTED` (`src/middleware/x402.ts`), un error tipado que además dice las dos salidas: otra cadena para el x402, o clave prepaga para seguir operando en `solana-devnet`. Con las envs puestas, ese mismo request contesta `402`. El flujo completo, el sobre y cada código de rechazo están en [doc/INTEGRATION.md](doc/INTEGRATION.md).
...
`.env.example` documenta **189 variables** con sus defaults (contadas con `grep -cE '^[A-Z][A-Z0-9_]*=' .env.example`, y esa misma cuenta la re-deriva `test/readme-numbers.test.ts` en cada `npm test`), y las pocas que cambian el comportamiento del dinero están agrupadas ahí.
...
| Métrica | Valor |
|---|---|
| Archivos de test | **312 archivos de test** en la suite raíz. Derivado del `include` de `vitest.config.ts` sobre el índice de git, y verificado en los dos README, por `test/readme-numbers.test.ts` |
...
| Lint | `npm run lint` (Biome) sobre **510 archivos**, que es el `src/**/*.ts` de `biome.json` |

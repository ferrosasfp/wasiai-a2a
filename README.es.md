...
| Métrica | Valor |
|---|---|
| Archivos de test | **291 archivos de test** en la suite raíz. Derivado del `include` de `vitest.config.ts` sobre el índice de git, y verificado en los dos README, por `test/readme-numbers.test.ts` |
...
| Lint | `npm run lint` (Biome) sobre **484 archivos**, que es el `src/**/*.ts` de `biome.json` |

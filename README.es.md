...
`.env.example` documenta **186 variables** con sus defaults (contadas con `grep -cE '^[A-Z][A-Z0-9_]*=' .env.example`, y esa misma cuenta la re-deriva `test/readme-numbers.test.ts` en cada `npm test`), y las pocas que cambian el comportamiento del dinero están agrupadas ahí.
...
| Métrica | Valor |
|---|---|
| Archivos de test | **303 archivos de test** en la suite raíz. Derivado del `include` de `vitest.config.ts` sobre el índice de git, y verificado en los dos README, por `test/readme-numbers.test.ts` |
...
| Lint | `npm run lint` (Biome) sobre **501 archivos**, que es el `src/**/*.ts` de `biome.json` |

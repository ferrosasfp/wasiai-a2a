...
describe('registerEventTracking middleware', () => {
  ...
  beforeAll(async () => {
    ...
    // Tracked endpoints
    app.post('/discover', async (_req: FastifyRequest, reply: FastifyReply) => reply.send({ agents: [] }))
    app.post('/orchestrate', async (_req: FastifyRequest, reply: FastifyReply) => reply.send({ result: 'ok' }))
    app.post('/compose', async (_req: FastifyRequest, reply: FastifyReply) => reply.send({ result: 'ok' }))
    app.post('/auth/agent-signup', async (_req: FastifyRequest, reply: FastifyReply) => reply.send({ ok: true }))
    app.get('/gasless/status', async (_req: FastifyRequest, reply: FastifyReply) => reply.send({ enabled: true }))

    // Non-tracked endpoints
    app.get('/health', async (_req: FastifyRequest, reply: FastifyReply) => reply.send({ status: 'ok' }))
    app.get('/dashboard/stats', async (_req: FastifyRequest, reply: FastifyReply) => reply.send({ stats: {} }))
    app.get('/', async (_req: FastifyRequest, reply: FastifyReply) => reply.send({ name: 'WasiAI' }))

    // Tracked endpoint returning 400 (for status mapping test)
    app.post('/discover/fail-test', async (_req: FastifyRequest, reply: FastifyReply) => reply.status(400).send({ error: 'bad' }))
  ...
  it('AC-1: POST /orchestrate — tracked', async () => {
    ...
    expect(mockTrack).toHaveBeenCalledTimes(1)
    expect(mockTrack.mock.calls[0][0].eventType).toBe('request:POST:/orchestrate')
  })
  ...
  it('AC-1: POST /auth/agent-signup — tracked', async () => {
    await app.inject({ method: 'POST', url: '/auth/agent-signup', payload: {} })
    await new Promise((r) => setTimeout(r, 50))

    expect(mockTrack).toHaveBeenCalledTimes(1)
    expect(mockTrack.mock.calls[0][0].eventType).toBe('request:POST:/auth/agent-signup')
  })
  ...
  it('AC-1: GET /gasless/status — tracked', async () => {
    ...
    expect(mockTrack).toHaveBeenCalledTimes(1)
    expect(mockTrack.mock.calls[0][0].eventType).toBe('request:GET:/gasless/status')
  })
  ...
  it('AC-3: track() error is swallowed — response still 200', async () => {
    ...
    const response = await app.inject({ method: 'POST', url: '/discover', payload: {} })
    await new Promise((r) => setTimeout(r, 50))
  ...
  it('AC-1: 4xx response → status: failed', async () => {
    await app.inject({ method: 'POST', url: '/discover/fail-test', payload: {} })
    await new Promise((r) => setTimeout(r, 50))

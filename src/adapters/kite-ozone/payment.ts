...
const EIP712_DOMAIN = { name: 'Kite x402', version: '1', chainId: kiteTestnet.id, verifyingContract: KITE_FACILITATOR_ADDRESS } as const
const EIP712_TYPES = { Authorization: [
...
export class KiteOzonePaymentAdapter implements PaymentAdapter {
  ...
  readonly supportedTokens: TokenSpec[] = [{ symbol: 'PYUSD', address: KITE_PAYMENT_TOKEN, decimals: 18 }]

  getScheme(): string { return KITE_SCHEME }
  getNetwork(): string { return KITE_NETWORK }
  getToken(): `0x${string}` { return KITE_PAYMENT_TOKEN }
  getMaxTimeoutSeconds(): number { return KITE_MAX_TIMEOUT_SECONDS }
  getMerchantName(): string { return process.env.KITE_MERCHANT_NAME ?? 'WasiAI' }
  ...
  async verify(proof: X402Proof): Promise<VerifyResult> {
    ...
    if (!response.ok) throw new Error(`Facilitator returned HTTP ${response.status} on /v2/verify`)
  ...
  async settle(req: SettleRequest): Promise<SettleResult> {
    ...
    if (!response.ok) throw new Error(`Facilitator returned HTTP ${response.status} on /v2/settle`)
    const result = (await response.json()) as PieverseSettleResult
    return { txHash: result.txHash, success: result.success, error: result.error }
  }

  async quote(_amountUsd: number): Promise<QuoteResult> {
    return { amountWei: '1000000000000000000', token: { symbol: 'PYUSD', address: KITE_PAYMENT_TOKEN, decimals: 18 }, facilitatorUrl: process.env.KITE_FACILITATOR_URL ?? KITE_FACILITATOR_DEFAULT_URL }
  }
  ...
  async sign(opts: SignRequest): Promise<SignResult> {
    ...
    const authorization = { from: account.address, to: opts.to, value: opts.value, validAfter: '0', validBefore: String(now + (opts.timeoutSeconds ?? 300)), nonce }
    const signature = await client.signTypedData({
      account, domain: EIP712_DOMAIN, types: EIP712_TYPES, primaryType: 'Authorization',
      message: { from: authorization.from, to: authorization.to, value: BigInt(authorization.value), validAfter: BigInt(authorization.validAfter), validBefore: BigInt(authorization.validBefore), nonce: authorization.nonce as `0x${string}` },
    })
    const paymentRequest: X402PaymentRequest = { authorization, signature, network: KITE_NETWORK }
    const xPaymentHeader = Buffer.from(JSON.stringify(paymentRequest)).toString('base64')
    return { xPaymentHeader, paymentRequest }
  }
}

export function _resetWalletClient(): void { _walletClient = null }

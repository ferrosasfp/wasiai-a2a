...
function sanitizeError(err: unknown): string {
  return err instanceof Error ? err.message.substring(0, 120) : 'unknown error';
}
...
export async function submitGaslessTransfer(payload: {
  from: `0x${string}`;
  to: `0x${string}`;
  value: string;
  validAfter: string;
...
export async function getGaslessStatus() {
  const adapter = new KiteOzoneGaslessAdapter();

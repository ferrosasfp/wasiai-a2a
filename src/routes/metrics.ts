...
const metricsRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.addHook('onRequest', async () => { activeRequests++ })
  ...
  fastify.get('/', { config: { rateLimit: false } }, async (_request: FastifyRequest, reply: FastifyReply) => {
    ...
    lines.push('# HELP wasiai_http_requests_total Total HTTP requests by route and method')
    lines.push('# TYPE wasiai_http_requests_total counter')
    for (const [key, s] of stats) {
      const [method, route] = key.split(' ', 2)
      lines.push(`wasiai_http_requests_total{method="${method}",route="${route}"} ${s.count}`)
    }

    // Per-route errors
    lines.push('# HELP wasiai_http_errors_total Total HTTP error responses (4xx/5xx)')
    lines.push('# TYPE wasiai_http_errors_total counter')
    for (const [key, s] of stats) {
      const [method, route] = key.split(' ', 2)
      if (s.errors > 0) {
        lines.push(`wasiai_http_errors_total{method="${method}",route="${route}"} ${s.errors}`)
      }
    }

    // Per-route latency histogram
    lines.push('# HELP wasiai_http_duration_ms HTTP request duration in milliseconds')
    lines.push('# TYPE wasiai_http_duration_ms histogram')
    for (const [key, s] of stats) {
      const [method, route] = key.split(' ', 2)
      for (const [bucket, count] of Object.entries(s.buckets)) {
        lines.push(`wasiai_http_duration_ms_bucket{method="${method}",route="${route}",le="${bucket}"} ${count}`)
      }
      lines.push(`wasiai_http_duration_ms_sum{method="${method}",route="${route}"} ${s.totalMs.toFixed(0)}`)
      lines.push(`wasiai_http_duration_ms_count{method="${method}",route="${route}"} ${s.count}`)
    }

...
export const eventService = {
  ...
  async stats(): Promise<DashboardStats> {
    ...
    if (regErr) throw new Error(`Failed to count registries: ${regErr.message}`)
    ...
    if (eventsErr) throw new Error(`Failed to get events: ${eventsErr.message}`)

    const events = eventsData ?? []
    const eventsTotal = events.length
    const successCount = events.filter((e: Record<string, unknown>) => e.status === 'success').length
    const successRate = eventsTotal > 0 ? (successCount / eventsTotal) * 100 : 0
      ...
      if (agentId) {
        ...
        if (existing) {
          ...
          agentMap.set(agentId, {
            ...
            invocations: 1,
            totalLatency: event.latency_ms != null ? Number(event.latency_ms) : 0,

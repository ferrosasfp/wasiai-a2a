...
describe('identityService', () => {
  ...
  describe('createKey', () => {
    ...
    it('stores SHA-256 hash of the plaintext key, not the plaintext', async () => {
      ...
      mock.insert = vi.fn().mockImplementation((row: Record<string, unknown>) => {
    ...
    it('throws on DB error', async () => {
      ...
      await expect(identityService.createKey({ owner_ref: 'x' }))
  ...
  describe('lookupByHash', () => {
    it('returns row when found', async () => {
      const fakeRow = { id: 'uuid-1', key_hash: 'abc', owner_ref: 'user-1', is_active: true }
      const mock = chainMock()
    ...
    it('throws on unexpected DB error', async () => {
      ...
      await expect(identityService.lookupByHash('x'))
        .rejects.toThrow('Failed to lookup agent key: unexpected')
    })
  })
  ...
  describe('deactivate', () => {
    ...
    it('throws on DB error', async () => {
      ...
      await expect(identityService.deactivate('x'))
        .rejects.toThrow('Failed to deactivate agent key: fail')
    })
  })
})

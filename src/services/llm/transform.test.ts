...
vi.mock('../../lib/supabase.js', () => {
  ...
  mockFrom.mockReturnValue({ select: mockSelect, eq: mockEq, single: mockSingle, update: mockUpdate, upsert: mockUpsert })
...
// Import after mocks
import { maybeTransform, _clearL1Cache } from './transform.js'
...
function getFromMock<T>(obj: unknown, ...path: string[]): T {
  ...
  return path.reduce((o: any, k) => o[k], obj) as T
}

function setupSupabaseMiss() {
  const single = getFromMock<ReturnType<typeof vi.fn>>(supabase.from('x'), 'single')
  single.mockResolvedValue({ data: null, error: { message: 'not found', details: '', hint: '', code: '404', name: 'PostgrestError' } })
}

function setupSupabaseHit(transformFn: string, hitCount = 0) {
  const single = getFromMock<ReturnType<typeof vi.fn>>(supabase.from('x'), 'single')
  single.mockResolvedValue({ data: { transform_fn: transformFn, hit_count: hitCount }, error: null, count: null, status: 200, statusText: 'OK' })
}
...
beforeEach(() => {
  ...
  const single = vi.fn().mockResolvedValue({
    data: null,
    error: { message: 'not found', details: '', hint: '', code: '404', name: 'PostgrestError' },
  })
...
describe('maybeTransform', () => {
  ...
  it('T-2: second call for same pair uses L1 cache (no LLM)', async () => {
    ...
    await maybeTransform('agent-a', 'agent-b', { text: 'hello' }, { required: ['query'] })

    vi.clearAllMocks()
    // Re-setup from mock but keep L1 (don't call _clearL1Cache)
    const single2 = vi.fn().mockResolvedValue({ data: null, error: { message: 'not found', details: '', hint: '', code: '404', name: 'PostgrestError' } })
    ...
    // Second call — L1 hit
    const result = await maybeTransform('agent-a', 'agent-b', { text: 'world' }, { required: ['query'] })
  ...
  it('T-4: compatible output skips transform entirely', async () => {
    ...
    const inputSchema = { required: ['query'], properties: { query: { type: 'string' } } }

    const result = await maybeTransform('agent-e', 'agent-f', output, inputSchema)

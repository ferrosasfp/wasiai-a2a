...
function makeListChain(result: { data: unknown; error: unknown }) {
  ...
  const terminal: Record<string, unknown> = {
    // biome-ignore lint/suspicious/noThenProperty: Supabase PostgREST builder is thenable by design
...
describe('taskService', () => {
  ...
  it('5. list() sin filtros retorna array de tasks', async () => {
    const rows = [
      makeTaskRow(),
      makeTaskRow({ id: 'bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb' }),
    ];
  ...
  it('15. append() messages los agrega (no reemplaza)', async () => {
    ...
    const updatedRow = makeTaskRow({
      messages: [{ text: 'original' }, { text: 'new' }],
    });

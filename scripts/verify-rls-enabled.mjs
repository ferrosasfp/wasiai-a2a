 ...
 *
 * El project ref NO se hardcodea acá: sale del argumento, de PROJECT_REF o de
 * SUPABASE_URL en el `.env` local (que no se versiona). Este repo es PÚBLICO.
 */
...
export async function main(deps = {}) {
  ...
  // Las rutas se derivan de la ubicación de ESTE archivo (repo root = ../), no de
  // una ruta absoluta de una máquina concreta: así funciona en CI y para cualquier
  // clon. `.env`/`.env.local` no se versionan; si faltan, readEnv devuelve {}.
  const env =
    deps.env ?? {
      ...readEnv(fileURLToPath(new URL('../.env', import.meta.url))),
      ...readEnv(fileURLToPath(new URL('../.env.local', import.meta.url))),
    };
